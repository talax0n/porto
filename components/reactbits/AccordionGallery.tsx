'use client';
/* eslint-disable @typescript-eslint/no-explicit-any, prefer-const -- vendored from reactbits.dev, kept close to upstream */
        ...
        return (
          ...
          >
            <span className="absolute inset-0 overflow-hidden [border-radius:inherit]">
              ...
              >
                {/* gradient always paints — the image sits on top and may fail to load */}
                <span
                  className="absolute inset-0 block"
                  style={{ background: item.gradient || '#141413' }}
                />
                {item.image && (
                  <img
                    src={item.image}
                    ...
                    draggable={false}
                    className="relative block h-full w-full select-none object-cover [-webkit-user-drag:none]"
                    onError={e => {
            ...
            {/* always-on corner tag — inactive panels are otherwise anonymous */}
            {showLabels && !isActive && (
              <span
                className="pointer-events-none absolute top-4 left-5 z-[2] max-w-[calc(100%-2.5rem)] overflow-hidden text-[11px] font-semibold tracking-[0.1em] text-ellipsis whitespace-nowrap uppercase opacity-60 [text-shadow:0_1px_8px_rgba(0,0,0,0.6)]"
                style={{ color: textColor, writingMode: vertical ? 'horizontal-tb' : 'vertical-rl' }}
                aria-hidden="true"
              >
                {item.label}
              </span>
            ...
            {showLabels && (
              ...
              >
                ...
                <span
                  ...
                  className="flex min-w-0 flex-col gap-1 opacity-0 [text-shadow:0_2px_14px_rgba(0,0,0,0.55)]"
                  style={{ color: textColor }}
                >
                  {(item.sublabel || item.badge) && (
                    <span className="flex items-center gap-2 whitespace-nowrap text-[10px] font-semibold tracking-[0.14em] uppercase opacity-70">
                      {item.sublabel}
                      {item.badge && (
                        <span className="rounded-full border border-current px-1.5 py-[1px] text-[8px] tracking-[0.08em]">
                          {item.badge}
                        </span>
                      )}
                    </span>
                  )}
                  <span className="overflow-hidden text-ellipsis whitespace-nowrap text-[clamp(1rem,1.4vw,1.5rem)] font-extrabold tracking-[-0.02em]">
                    {item.label}
                  </span>
                </span>
              </span>
